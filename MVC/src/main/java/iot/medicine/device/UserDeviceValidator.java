...
//        if (device.getSerialNumber() == null || device.getSerialNumber()<8 || device.getSerialNumber() > 12){
        if (!m.matches()){
            errors.rejectValue("serialNumber", "serialNumber.null", "Неверный S/N");
        }
        ...
        if (ifExist != null || ifExist.getSerialNumber().equals(device.getSerialNumber())){
            errors.rejectValue("serialNumber", "serialNumber.null", "Устройство уже зарегестрировано");
        }
    }
}
